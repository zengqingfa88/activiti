...
var KisBpmAssignmentPopupCtrl = ['$scope', '$http', function ($scope, $http) {
    ...
    //异步请求表格数据
    $scope.getPagedDataAsync = function (pageNo, pageSize, projectId) {
        var url;
        if ($scope.selectType == 2) {
            url = '/kft_activiti_demo_war_exploded/group/getGroupList';
            $scope.columnData = $scope.groupColumns;
        } else {
            url = '/kft_activiti_demo_war_exploded/user/getUserList';
        ...
        $http({
            ...
            $scope.gridData = response.data.content;
            $scope.totalServerItems = response.data.totalElements;
        }, function errorCallback(response) {
            // 请求失败执行代码
            $scope.gridData = [];
